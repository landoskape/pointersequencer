...
def train(nets, optimizers, dataset, **parameters):
    ...
    for epoch in epoch_loop:
        ...
        scores, choices, _ = forward_batch(nets, batch, temperature=temperature.get_value(), thompson=thompson)

        # get baseline choices if using them
        if baseline:
            with torch.no_grad():
                bl_choices = forward_batch(bl_nets, batch, temperature=bl_temperature, thompson=bl_thompson)[1]
